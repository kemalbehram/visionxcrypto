...
class PaymentController extends Controller
{
    ...
    public function depositConfirm(Request $request)
    {
        ...
        if (is_null($data)) {
            return redirect()->route('deposit')->with('success', 'Invalid Deposit Request');
        }
        if ($data->status != 0) {
            return redirect()->route('deposit')->with('success', 'Invalid Deposit Request');
        }
         ...
         if ($request->bank) {
            ...
            $data->save();
               ...
               return redirect()->route('deposit')->with('success', 'Your Deposit Request Received.Please wait while we vlidate your payment');
        }
    ...
    public function ipnperfect()
    {
        ...
        if ($hash == $hash2) {
            ...
            if ($_POST['PAYEE_ACCOUNT'] == $gatewayData->val1 && $unit == "USD" && $amo == $total && $data->status == '0') {
                //Update User Data
                $this->userDataUpdate($data);
            }
        }
