...
@section('body')
    <div class="page-content"><div class="container"><div class="card content-area"><div class="card-innr"><div class="card-head"><h4 class="card-title">Cryptocurrency Trade Plan</h4></div><table class="data-table dt-init user-list"><thead><tr class="data-item data-head"><th class="data-col dt-user">Name</th><th class="data-col dt-email">Min-Max</th><th class="data-col dt-token">Rate</th>  <th class="data-col dt-status"><div class="dt-status-text">Status</div></th><th class="data-col"></th></tr></thead><tbody>
<a href="{{route('plan-create')}}"  class="btn btn-sm btn-primary btn-outline"><em class="ti ti-plus"></em> Create New</a>
@foreach($plan as $data)
<tr class="data-item"><td class="data-col dt-user"><span class="lead user-name">{{$data->name }}</span><span class="sub user-id"></span></td><td class="data-col dt-email"><span class="sub sub-s2 sub-email"><strong> @if($data->fixed_amount == 0) {{$basic->currency_sym}} {{$data->minimum}}
                                                    - {{$basic->currency_sym}} {{$data->maximum}}  @else {{$basic->currency_sym}} {{$data->maximum}} @endif</strong></span></td><td class="data-col dt-token"><span class="lead lead-btoken"> 
     <strong>{{$data->interest}} @if($data->interest_status == 1)
                                                    % @else {{$basic->currency_sym}} @endif  Interest</span></td>
...
</td><td class="data-col text-right"><div class="relative d-inline-block"><a href="#" class="btn btn-light-alt btn-xs btn-icon toggle-tigger"><em class="ti ti-more-alt"></em></a><div class="toggle-class dropdown-content dropdown-content-top-left"><ul class="dropdown-list"><li><a href="{{route('plan-edit',$data->id)}}"><em class="ti ti-eye"></em> Update Details</a></li> </ul></div></div></td></tr>
@endforeach
