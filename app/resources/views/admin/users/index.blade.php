...
@section('body')
   <div class="page-content"><div class="container"><div class="card content-area"><div class="card-innr"><div class="card-head"><h4 class="card-title">User List</h4></div><table class="data-table dt-init user-list"><thead><tr class="data-item data-head"><th class="data-col dt-user">User</th><th class="data-col dt-email">Email</th><th class="data-col dt-token">Total TRX</th><th class="data-col dt-verify">Verification Status</th><th class="data-col dt-login">Registered Date</th><th class="data-col dt-status"><div class="dt-status-text">Status</div></th><th class="data-col"></th></tr></thead><tbody>

  @foreach($users as $user)
<tr class="data-item"><td class="data-col dt-user"><span class="lead user-name">{{$user->fname}} {{$user->lname}}</span><span class="sub user-id">{{$user->username}}</span></td><td class="data-col dt-email"><span class="sub sub-s2 sub-email">{{$user->email}}</span></td><td class="data-col dt-token"><span class="lead lead-btoken">{{$basic->currency}}{{number_format(App\Trx::whereUser_id($user->id)->whereStatus(2)->sum('main_amo'), $basic->decimal)}}</span></td><td class="data-col dt-verify"><ul class="data-vr-list"><li>
