...
@section('body')
    <div class="page-content"><div class="container"><div class="card content-area"><div class="card-innr"><div class="card-head"><h4 class="card-title">Withdrawal Log</h4></div><table class="data-table dt-init user-tnx"><thead><tr class="data-item data-head"><th class="data-col dt-tnxno">Tranx NO</th><th class="data-col dt-token">Amount</th><th class="data-col dt-amount">Gateway</th><th class="data-col dt-usd-amount">Charge</th><th class="data-col dt-type"><div class="dt-type-text">Status</div></th><th class="data-col"></th></tr></thead><tbody>
...
@endif</div><div class="fake-class"><span class="lead tnx-id">{{isset($data->transaction_id ) ? $data->transaction_id  : 'N/A'}}</span><span class="sub sub-date">{!! date(' d/M/Y', strtotime($data->created_at)) !!}</span></div></div></td><td class="data-col dt-token"><span class="lead token-amount"> {{$data->amount}}</span><span class="sub sub-symbol">{!! $basic->currency !!}</span></td><td class="data-col dt-amount"><span class="lead amount-pay"> {{isset($data->gateway->name) ? $data->gateway->name : 'N/A'}}</span></td><td class="data-col dt-usd-amount"><span class="lead amount-pay">{{$data->charge}}</span><span class="sub sub-symbol">{{$basic->currency}} <em class="fas fa-info-circle" data-toggle="tooltip" data-placement="bottom" ></em></span></td><td class="data-col dt-type">
@if($data->status > 0)
...
</div><div class="fake-class"><span class="data-details-title">Payment Gateway</span><span class="data-details-info">{{isset($data->gateway->name) ? $data->gateway->name : 'N/A'}}</span></div></div><div class="gaps-3x"></div><h6 class="card-sub-title">Transaction Info</h6><ul class="data-details-list"><li><div class="data-details-head">Transaction Type</div><div class="data-details-des"><strong>Withdrawal</strong></div></li><!-- li --

<li><div class="data-details-head">Tx Hash</div><div class="data-details-des"><span>{{$data->transaction_id}}</span> <span></span></div></li><!-- li --><li><div class="data-details-head">Transaction ID</div><div class="data-details-des"><span>{{$data->transaction_id}}</span> <span></span></div></li><!-- li --><li><div class="data-details-head">Amount</div><div class="data-details-des">{!! $basic->currency !!} {{$data->amount}}</div></li>
