		...
		<div class="main-page-wrapper user-access-page">
			...
			<div class="main-container">
				<div class="inner-box">
					...
					<div class="confirmation-text font-fix">A text message with a verification code was just sent to <span class="mark-text">{{Auth::user()->phone}}</span>. Please enter the code in the field below to verify your identity</div>
					...
					@csrf
					<div class="condition-text text-center font-fix">Haven't received the code yet? <a href="{{route('user.send-vcode') }}" onclick="event.preventDefault(); document.getElementById('requestphone').submit();" >Resend code</a></div>
					</form>
				</div> <!-- /.inner-box -->
			</div> <!-- /.main-container -->
		...
		<div class="main-page-wrapper user-access-page">
			...
			<div class="main-container">
				<div class="inner-box">
					...
					<div class="confirmation-text font-fix">Your account has been locked for using a wrong transaction password 3 time. Please close session and contact one of our support through our contact us page for assistance</div>

					<a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
					<button type="submit" class="theme-button"><span></span>Close Session</button></a>
				...
				<div id="main-top-header">
					<div class="container">
						<div class="wrapper clearfix">
							...
							<a href="{{route('home')}}" class="logo-wrapperme"><img width="150" src="{{asset('assets/images/logo/logo.png')}}" alt=""></a>
                            </div>

                            <div class="hidden-lg-up hidden-sm-up hidden-md-up hidden-xl-up">
                                <a href="{{route('home')}}" class="logo-wrapper"><img width="100" src="{{asset('assets/images/logo/logo.png')}}" alt=""></a>
                            </div>
							<!-- Top Bar Action List -->
									...
									<li class="action-list-item balance-action-button">
										<div class="dropdown">
											<button class="dropdown-toggle" data-toggle="dropdown">
											    <span class="font-fix">{{$basic->currency_sym}}{{number_format(Auth::user()->balance, $basic->decimal)}} </span>
											</button>
													...
													<ul>
														<li class="bitcoin-method clearfix">
															<a class="clearfix" href="#">
																<span class="name font-fix"><img width="40" src="{{url('assets/images/invest.png')}}" alt=""> Investment Vault</span>
																...
																<span class="currency-title">{{number_format(App\UserWallet::where('user_id', Auth::id())->where('type', 'interest_wallet')->first()->balance * $bbttcc,7)}} BTC</span>
																	<span class="total-currency">${{number_format(App\UserWallet::where('user_id', Auth::id())->where('type', 'interest_wallet')->first()->balance,2)}}</span>
																</span>
															</a>
														</li>
														...
														<li class="litecoin-method clearfix">
															<a class="clearfix" href="#">
																<span class="name font-fix"><img width="40" src="{{asset('assets/images/naira.jpeg')}}" alt=""> Naira Wallet</span>
																<span class="balance-inquery">
										...
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right">
												<div class="user-account-information">
													 ...
													 @if( file_exists(Auth::User()->image))
												<img src="{{asset(Auth::user()->image)}}" alt="" class="user-lg-img">
												@else
												<img src="{{url('assets/user/images/user-default.png')}}" alt="" class="user-lg-img">
											    @endif
													<div class="user-name font-fix">{{Auth::user()->fname}} {{Auth::user()->lname}}</div>
													<div class="user-email">{{Auth::user()->email}}</div>
													<div class="member-rank">Member</div>
												</div> <!-- /.user-account-information -->
												<ul class="user-useful-link">
													<li><a href="{{route('profile')}}" class="font-fix">Your Profile</a></li>
													<li><a href="{{route('activities')}}" class="font-fix">Activity Log</a></li>
													<li><a href="{{route('inbox')}}" class="font-fix">Messages @if($ncount > 0)<span class="free-button">{{$ncount}} New</span> @endif </a></li>
													<li><a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="font-fix">Log Out</a>
					...
					<div id="dashboard-main-body" class="clearfix">
						...
						<div class="dashboard-sidebar-navigation">
							...
							<ul class="navigation-link" >
								<li class="{{ Request::segment(2) === 'home' ? 'active-link' : null }}"><a href="{{route('home')}}"><img src="{{url('assets/images/dashboard.svg')}}" alt="" class="svg"> Dashboard</a></li>
								<li class="{{ Request::segment(2) === 'coin-vest' ? 'active-link' : null }} {{ Request::segment(2) === 'coinvestyield' ? 'active-link' : null }}"><a href="{{route('coinvest')}}"><img src="{{url('assets/images/invest.png')}}" alt="" class="svg size-fix"> VX Vault</a></li>
								<li class="{{ Request::segment(2) === 'my-wallet' ? 'active-link' : null }} {{ Request::segment(2) === 'transfer' ? 'active-link' : null }}"><a href="{{route('my-wallet')}}"><img src="{{url('assets/images/wallet.png')}}" alt="" class="svg size-fix"> Wallet</a></li>
								<li class="{{ Request::segment(2) === 'products' ? 'active-link' : null }} {{ Request::segment(2) === 'trade-coin' ? 'active-link' : null }}{{ Request::segment(2) === 'airtime' ? 'active-link' : null }} {{ Request::segment(2) === 'internet' ? 'active-link' : null }} {{ Request::segment(2) === 'pay-tv-bills' ? 'active-link' : null }} {{ Request::segment(2) === 'utilitybills' ? 'active-link' : null }} {{ Request::segment(2) === 'instantsms' ? 'active-link' : null }}"><a href="{{route('products')}}"><img src="{{url('assets/images/cart.png')}}" alt="" class="svg"> Products</a></li>
								<li class="{{ Request::segment(2) === 'visionxcard' ? 'active-link' : null }}"><a href="{{route('showvcard')}}"><img src="{{url('assets/images/airtime.jpg')}}" alt="" class="svg"> VXCard</a></li>
								<li class="{{ Request::segment(2) === 'edit-profile' ? 'active-link' : null }} {{ Request::segment(2) === 'activity-log' ? 'active-link' : null }} {{ Request::segment(2) === 'inbox' ? 'active-link' : null }}"><a href="{{route('profile')}}"><img src="{{url('assets/images/settings.png')}}" alt="" class="svg"> Settings</a></li>
								<li class="{{ Request::segment(2) === 'verification' ? 'active-link' : null }}"><a href="{{route('verification')}}"><img src="{{url('assets/images/shield.png')}}" alt="" class="svg"> Verification</a></li>
							</ul>
